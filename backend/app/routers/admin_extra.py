...
def decide_approval(kind: str, obj_id: int, body: dict,
    ...
    if obj.user:
        notify(db, obj.user.id, "system",
               ...
               notify_service.user_link(db, obj.user.id, "profile"), commit=False)
    db.commit()
...
def subscribe(body: dict, db: Session = Depends(get_db)):
    ...
    notify(db, user.id, "system", "Subscription updated", msg,
           notify_service.user_link(db, user.id, "billing"), commit=False)
    db.commit()
    return {"message": msg}
