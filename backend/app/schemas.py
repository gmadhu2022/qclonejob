...
class InstituteRegister(InstituteBase):
    """Self-registration from the institute registration page.

    logo_url sits here and not on InstituteBase for the same reason as
    EnterpriseRegister: the profile/admin PUT endpoints overwrite every field
    they receive, so a form that didn't resend logo_url would wipe the logo.
...
class EnterpriseRegister(EnterpriseBase):
    ...
    logo_url is declared here rather than on EnterpriseBase on purpose: the
    profile/admin PUT endpoints reuse EnterpriseBase and overwrite every field
    they receive, so a form that didn't send logo_url would wipe an existing
...
class JobSeekerBase(BaseModel):
    prefer_uploaded_resume: Optional[bool] = None
    resume_additions: Optional[List[dict]] = None
...
class JobSeekerOut(JobSeekerBase):
    ...
    # Read-only here: the file is set by the upload endpoint, so a profile PUT
    # that omits it can't wipe an uploaded resume.
    uploaded_resume_url: Optional[str] = None
    uploaded_resume_name: Optional[str] = None
